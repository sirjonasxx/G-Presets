...
public class GPresetImporter {
    ...
    public enum BuildingImportState {
        ...
        // select a location ingame that will be used to move furniture to that need state
        // adjustments
        // preferably your character stands next to it (for manipulating states for furni like
        // switches)
    ...
    public GPresetImporter(GPresets extension) {
        ...
        extension.intercept(
                ...
                this::maybeBlockPlacements);

        extension.intercept(HMessage.Direction.TOSERVER, "MoveObject", this::blockFurniAdjustments);
        extension.intercept(
                HMessage.Direction.TOSERVER, "UseFurniture", this::blockFurniAdjustments);
        extension.intercept(
                HMessage.Direction.TOSERVER,
                "SetCustomStackingHeight",
                this::blockFurniAdjustments);
        extension.intercept(
                HMessage.Direction.TOSERVER, "UpdateCondition", this::blockFurniAdjustments);
        extension.intercept(
                HMessage.Direction.TOSERVER, "UpdateTrigger", this::blockFurniAdjustments);
        extension.intercept(
                HMessage.Direction.TOSERVER, "UpdateAction", this::blockFurniAdjustments);
        extension.intercept(
                HMessage.Direction.TOSERVER, "UpdateAddon", this::blockFurniAdjustments);
        extension.intercept(
                HMessage.Direction.TOSERVER, "UpdateSelector", this::blockFurniAdjustments);
        ...
        if (state == BuildingImportState.SETUP_WIRED) {
            ...
            if (workingPresetConfig != null && workingPresetConfig.getPresetWireds() != null) {
                HashMap<String, String> map =
                        workingPresetConfig.getPresetWireds().getVariablesMap();

                for (HWiredVariable newVar : variables) {
                    Optional<Map.Entry<String, String>> op =
                            map.entrySet().stream()
                                    .filter(k -> k.getKey().equals(newVar.name))
                                    .findFirst();
                    op.ifPresent(
                            stringStringEntry ->
                                    realVariableIdMap.put(stringStringEntry.getValue(), newVar.id));
                }

                for (PresetWiredVariable origVar :
                        workingPresetConfig.getPresetWireds().getVariables()) {
                    for (HWiredVariable newVar : variables) {
        ...
        synchronized (lock) {
            if (state == BuildingImportState.SETUP_WIRED
                ...
                extension.sendVisualChatInfo(
                        "Don't adjust furniture while the importer is active!");
            }
        }
    }
    ...
    private void maybeBlockPlacements(HMessage hMessage) {
        synchronized (lock) {
            if (state != BuildingImportState.NONE) {
                ...
                extension.sendVisualChatInfo(
                        "Can't drop furniture while the importer is active, please await the procedure or abort");
            }
        }
    }

    private boolean isPlacing() {
        return state == BuildingImportState.ADD_UNSTACKABLES
    ...
    private void onObjectAdd(HMessage hMessage) {
        synchronized (lock) {
            if (isPlacing()) {
                ...
                //                String classname =
                // extension.getFurniDataTools().getFloorItemName(item.getTypeId());
                String dropKey =
                        String.format(
                                "%d|%d|%d",
                ...
                if (awaitMatchingDropIds != null && awaitMatchingDropIds.size() > 0) {
                    ...
                    //                    if (classname != null &&
                    // classname.equals("ads_background")) {
                    //                        PresetAdsBackground ads =
                    // workingPresetConfig.getAdsBackgrounds().stream().filter(a -> a.getFurniId()
                    // == assignedFurniId).findFirst().orElse(null);
                    //                        if (ads != null) {
                    //
                    //                        }
                    //                    }
    ...
    public boolean isReady() {
        return extension.getFloorState().inRoom()
                && extension.furniDataReady()
                && extension.getInventory().getState() == Inventory.InventoryState.LOADED
                && extension.stackTile() != null
                && presetConfig != null
                && extension.getPermissions().canMoveFurni()
                && (!extension.shouldExportWired() || extension.getPermissions().canModifyWired());
    }
    ...
    private void onChat(HMessage hMessage) {
        synchronized (lock) {
            ...
            if (text.startsWith(":ip") || text.startsWith(":importpreset")) {
                ...
                if (state != BuildingImportState.NONE) {
                    extension.sendVisualChatInfo(
                            "Already importing preset.. finish up or abort first");
                } else if (!isReady()) {
    ...
    private void prepare() {
        if (presetConfig != null) {
            ...
            Map<String, Integer> missing =
                    AvailabilityChecker.missingItems(
                            fakeDropInfo, extension.getInventory(), furniData);
            if (missing == null) {
                extension.sendVisualChatInfo(
                ...
                Set<String> allStacktileClasses =
                        Arrays.stream(StackTileSetting.values())
                                .map(StackTileSetting::getClassName)
                                .collect(Collectors.toSet());
                allStacktileClasses.forEach(
                        c -> {
                            List<HFloorItem> stackTiles =
                                    extension.getFloorState().getItemsFromType(furniData, c);
                            if (stackTiles.size() > 0) {
                                HFloorItem stackTile = stackTiles.get(0);
                                allAvailableStackTiles.add(
                                        new StackTileInfo(
                                                stackTile.getId(),
                                                stackTile.getTile(),
                ...
                if (allAvailableStackTiles.size() > 0) {
                    extension
                            ...
                            .log(
                                    String.format(
                                            "Detected %d available types of stacktiles",
                                            allAvailableStackTiles.size()),
                                    "green");
                }
            ...
            if (state == BuildingImportState.AWAITING_UNOCCUPIED_SPACE) {
                ...
                rootLocation =
                        new HPoint(
                                x, y // ,
                                //
                                // PresetUtils.lowestFloorPoint(extension.getFloorState(),
                                // workingPresetConfig, new HPoint(x, y))
                                );
            ...
            if (startAddingFurni) {
                heightOffset =
                        PresetUtils.lowestFloorPoint(
                                extension.getFloorState(), workingPresetConfig, rootLocation);
    ...
    private void dropFurni(FurniDropInfo dropInfo) {
        ...
        boolean useBC =
                source == ItemSource.ONLY_BC
                        || (source == ItemSource.PREFER_BC && offerId != -1)
        ...
        if (useBC) {
            if (offerId == -1) {
                if (!extension.allowIncompleteBuilds()) {
                    ...
                    extension.sendVisualChatInfo(
                            String.format(
                                    "ERROR: Couldn't find the item '%s' in BC warehouse.. aborting",
                                    className));
                } else {
                    extension.sendVisualChatInfo(
                            String.format(
                                    "Couldn't find the item '%s' in BC warehouse.. continuing",
                                    className));
                }
                return;
            }
            ...
            if (inventoryItems.size() == 0) {
                if (!extension.allowIncompleteBuilds()) {
                    ...
                    extension.sendVisualChatInfo(
                            String.format(
                                    "ERROR: Couldn't find '%s' in inventory.. aborting",
                                    className));
                } else {
                    extension.sendVisualChatInfo(
                            String.format(
                                    "Couldn't find '%s' in inventory.. continuing", className));
                }
                return;
            }
    ...
    private void moveFurni(
            int furniId, int x, int y, int rot, boolean moveStackTile, double height) {
        if (moveStackTile) {
            StackTileInfo stackInfo =
                    StackTileUtils.findBestDropLocation(
                            x, y, allAvailableStackTiles, extension.getFloorState());
            if (stackInfo != null) {
                moveFurni(
                        stackInfo.getFurniId(),
                        stackInfo.getLocation().getX(),
                        stackInfo.getLocation().getY(),
                        stackInfo.getRotation(),
                        false,
                        -1);
                if (height != -1) {
                    extension.sendToServer(
        ...
        extension.sendToServer(
                new HPacket("MoveObject", HMessage.Direction.TOSERVER, furniId, x, y, rot));
    ...
    private void attemptSetState(int furniId, String targetState) {
        ...
        if (extension.getPermissions().canModifyWired()) {
            ...
            int i = 0;
            while (state != BuildingImportState.NONE
                    && newState != null
    ...
    private void saveWired(PresetWiredBase presetWired, int attempt) {
        ...
        if (presetWired.getVariableIds() != null && presetWired.getVariableIds().size() > 0) {
            ...
            needVariableIds =
                    needVariableIds.stream()
                            .filter(
                                    id ->
                                            id.equals("0")
                                                    && realVariableIdMap.keySet().stream()
            ...
            if (needVariableIds.size() > 0) {
                ...
                try {
                    gotVariableConfirmation =
                            wiredVariableConfirmation.tryAcquire(5, TimeUnit.SECONDS);
        ...
        synchronized (lock) {
            presetWiredBase =
                    presetWired.applyWiredConfig(extension, realFurniIdMap, realVariableIdMap);
        }

        if (presetWired instanceof PresetWiredCondition)
            latestConditionSave = System.currentTimeMillis();
        else if (presetWired instanceof PresetWiredEffect)
            latestEffectSave = System.currentTimeMillis();
        else if (presetWired instanceof PresetWiredTrigger)
            latestTriggerSave = System.currentTimeMillis();
        else if (presetWired instanceof PresetWiredAddon)
            latestAddonSave = System.currentTimeMillis();
        else if (presetWired instanceof PresetWiredSelector)
            latestSelectorSave = System.currentTimeMillis();
        else if (presetWired instanceof PresetWiredVariable)
            latestVariableSave = System.currentTimeMillis();

        boolean gotConfirmation = false;
        try {
            gotConfirmation = wiredSaveConfirmation.tryAcquire(5, TimeUnit.SECONDS);
        } catch (InterruptedException e) {
        }
    ...
    private void moveFurniture() {
        ...
        synchronized (lock) {
            workingPresetConfig
                    ...
                    .forEach(
                            p -> {
                                if (furniData.isStackable(p.getClassName())
                                        && !p.getClassName().startsWith("wf_trg_")
                                        && realFurniIdMap.containsKey(p.getFurniId())) {
                                    moveList.add(p);
                                }
                            });
            workingPresetConfig
        ...
        if (state == BuildingImportState.MOVE_FURNITURE) {
            List<HFloorItem> maybeNotMovedItems =
                    extension
                            .getFloorState()
                            .getFurniOnTile(stackTileLocation.getX(), stackTileLocation.getY());
            maybeNotMovedItems.addAll(
                    extension
                            .getFloorState()
                            .getFurniOnTile(stackTileLocation.getX(), stackTileLocation.getY()));
            maybeNotMovedItems.addAll(
                    extension
                            .getFloorState()
                            .getFurniOnTile(reservedSpace.getX(), reservedSpace.getY()));
            maybeNotMovedItems =
                    maybeNotMovedItems.stream()
                            .filter(
                                    h -> {
                                        PresetFurni p = movesByRealId.get(h.getId());
                                        if (p == null) return false;
                                        return p.getLocation().getX() + rootLocation.getX()
                                                        != h.getTile().getX()
        ...
        if (state == BuildingImportState.MOVE_FURNITURE) {
            for (StackTileInfo stackTileInfo : allAvailableStackTiles) {
                moveFurni(
                        stackTileInfo.getFurniId(),
                        stackTileInfo.getLocation().getX(),
                        stackTileInfo.getLocation().getY(),
                        stackTileInfo.getRotation(),
                        false,
                        -1);
            }
        }
    ...
    private void setupWired() {
        ...
        synchronized (lock) {
            ...
            int maxSize =
                    Math.max(
                            presetWireds.getConditions().size(),
                            Math.max(
                                    presetWireds.getEffects().size(),
                                    Math.max(
                                            presetWireds.getTriggers().size(),
                                            Math.max(
            ...
            Predicate<PresetWiredBase> filter =
                    (wiredBase) -> realFurniIdMap.containsKey(wiredBase.getWiredId());

            List<PresetWiredVariable> variables =
                    presetWireds.getVariables().stream()
                            .filter(filter)
                            .collect(Collectors.toList());
            allWireds.addAll(variables);
            allWireds.addAll(
                    presetWireds.getConditions().stream()
                            .filter(filter)
                            .collect(Collectors.toList()));
            allWireds.addAll(
                    presetWireds.getEffects().stream().filter(filter).collect(Collectors.toList()));
            allWireds.addAll(
                    presetWireds.getTriggers().stream()
                            .filter(filter)
                            .collect(Collectors.toList()));
            allWireds.addAll(
                    presetWireds.getAddons().stream().filter(filter).collect(Collectors.toList()));
            allWireds.addAll(
                    presetWireds.getSelectors().stream()
                            .filter(filter)
                            .collect(Collectors.toList()));
            allWireds.addAll(
                    presetWireds.getAddons().stream().filter(filter).collect(Collectors.toList()));
            ...
            workingPresetConfig
                    ...
                    .forEach(
                            b -> {
                                if (!wiredBindings.containsKey(b.getWiredId()))
                                    wiredBindings.put(b.getWiredId(), new ArrayList<>());
        ...
        while (state == BuildingImportState.SETUP_WIRED && i < allWireds.size()) {
            ...
            if ((i + 1) % 50 == 0) {
                extension
                        ...
                        .log(
                                String.format(
                                        "Setting up wired.. (%d/%d)", i + 1, allWireds.size()),
                                "orange");
            }
                    ...
                    synchronized (lock) {
                        ...
                        if (needMovement) {
                            ...
                            if (floorItem != null) {
                                ...
                                FurniMoveInfo undoMovement =
                                        new FurniMoveInfo(
                                                furniId,
                                                floorItem.getTile().getX(),
                                                floorItem.getTile().getY(),
                                                floorItem.getFacing().ordinal(),
                                                null,
                                                needStacktile);
            ...
            for (FurniMoveInfo furniMoveInfo : bindMoves) {
                double altitude =
                        furniMoveInfo.getAltitude() == null
                                ? -1
                                : ((double) furniMoveInfo.getAltitude()) / 100;
                moveFurni(
                        furniMoveInfo.getFurniId(),
                        furniMoveInfo.getX(),
                        furniMoveInfo.getY(),
                        furniMoveInfo.getRotation(),
                        furniMoveInfo.useStackTile(),
                        altitude);
            }
            ...
            for (FurniMoveInfo furniMoveInfo : undoBindMoves) {
                ...
                moveFurni(
                        ...
                        furniMoveInfo.useStackTile(),
                        altitude);
                // todo maybe set to original height? perhaps needed when furni is overridden with
                // existing furni
            }
        }
        ...
        while (i < furniDropInfos.size() && state == BuildingImportState.ADD_FURNITURE) {
            ...
            if ((i + 1) % 50 == 0) {
                extension
                        ...
                        .log(
                                String.format(
                                        "Dropping furni.. (%d/%d)", i + 1, furniDropInfos.size()),
                                "orange");
            }
    ...
    private void setupAds() {
        ...
        if (!adsBackgrounds.isEmpty()) {
            ...
            while (index < adsBackgrounds.size() && state == BuildingImportState.SETUP_ADS) {
                ...
                synchronized (lock) {
                    if (state == BuildingImportState.SETUP_ADS
                            && realFurniIdMap.containsKey(adsBackground.getFurniId())) {
                        ...
                        extension.sendToServer(
                                new HPacket(
                                        ...
                                        8,
                                        "imageUrl",
                                        adsBackground.getImageUrl(),
                                        "offsetX",
                                        adsBackground.getOffsetX(),
                                        "offsetY",
                                        adsBackground.getOffsetY(),
                                        "offsetZ",
                                        adsBackground.getOffsetZ()));
                    }
                }
    ...
    private boolean locationContainsWired(int x, int y) {
        ...
        return floor.getFurniOnTile(x, y).stream()
                .anyMatch(
                        i -> {
