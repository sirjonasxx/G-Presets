...
public class GPresets extends ExtensionForm {
    ...
    public void initialize() {
        ...
        stacktile_tgl
                ...
                .addListener(
                        observable -> {
                            String option =
                                    ((RadioButton) (stacktile_tgl.getSelectedToggle())).getText();
        ...
        allowIncompleteBuildsCbx
                .selectedProperty()
                .addListener(
                        observable ->
                                Cacher.put(
                                        "allowIncompleteBuilds",
                                        allowIncompleteBuildsCbx.isSelected()));
        ...
        TableColumn<FurniPostConfig, Integer> existingFurniIdColumn =
                new TableColumn<>("Existing furni ID");
    ...
    protected void initExtension() {
        ...
        presetListView.setOnMouseClicked(
                event -> {
    ...
    private void loadCache() {
        ...
        stacktile_tgl.getToggles().stream()
        ...
        item_src_tgl.getToggles().stream()
    ...
    public HFloorItem stackTile() {
        ...
        List<HFloorItem> items =
                floorState.getItemsFromType(furniDataTools, stackTileSetting.getClassName());
    ...
    //        return isConnected && floorState.inRoom() && furniDataReady() && inventory.getState()
    // == Inventory.InventoryState.LOADED
    //                && stackTile() != null;
    //    }
    //
    ...
    private void updateLabel(Label lbl, boolean isFullfilled, boolean isBusy, boolean isOptional) {
        lbl.getStyleClass()
                .removeAll(
                        lbl.getStyleClass().stream()
                                .filter(p -> p.startsWith("lbl"))
                                .collect(Collectors.toList()));
        lbl.getStyleClass()
                .add(
                        isFullfilled
                                ? "lblgreen"
                                : (isBusy ? "lblorange" : (isOptional ? "lblgrey" : "lblred")));
    }
    ...
    private void updateUI() {
        Platform.runLater(
                () -> {
                    ...
                    updateLabel(
                            cndInventoryLbl,
                            inventory.getState() == Inventory.InventoryState.LOADED,
                            inventory.getState() == Inventory.InventoryState.LOADING);
                    updateLabel(cndStackTileLbl, stackTile() != null);
                    updateLabel(
                            cndPermissionsLbl,
                            permissions.canMoveFurni()
                                    && (noExportWiredCbx.isSelected()
                                            || permissions.canModifyWired()));

                    availabilityBtn.setDisable(importer.getPresetConfig() == null);
                    currentPresetBtn.setDisable(
                            presetListView.getSelectionModel().getSelectedItem() == null);
                });
    }
    ...
    private PostConfig createPostConfig() {
        ...
        ItemSource itemSource =
                ItemSource.valueOf((String) item_src_tgl.getSelectedToggle().getUserData());
            ...
            for (PresetFurni f : combined.getFurniture()) {
                fakeDropInfo.add(
                        new FurniDropInfo(
                                -1,
                                -1,
                                furniDataTools.getFloorTypeId(f.getClassName()),
        ...
        if (selectedPreset != null) {
            try {
                Desktop.getDesktop()
                        .edit(
                                new File(
                                        PresetConfigUtils.presetPath(),
                                        selectedPreset + PresetConfigUtils.PRESET_EXT));
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    ...
    public void updatePostconfigClick(ActionEvent actionEvent) {
        try {
            if (furniNamePC_txt.getText().isEmpty() || replacementIdPC_txt.getText().isEmpty())
                throw new Exception();

            FurniPostConfig furniPostConfig =
                    new FurniPostConfig(
                            furniNamePC_txt.getText(),
                            true,
                            Integer.parseInt(replacementIdPC_txt.getText()),
                            null,
                            null,
                            null,
                            null,
                            ...
                            new HashMap<>());
            furniPostConfigs =
                    furniPostConfigs.stream()
                            .filter(
                                    c ->
                                            !c.getFurniIdentifier()
                                                    .equals(furniPostConfig.getFurniIdentifier()))
                            .collect(Collectors.toList());
